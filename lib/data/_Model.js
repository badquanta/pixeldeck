...
class _Model {
  ...
  constructor (json = {}) {
    ...
    if (!json.$loki) {
      ...
      this.constructor.dbg(
        'initializing new reference to: %o',
        Object.keys(json)
      )
      record = json
    }
  ...
  static find (json) {
    const query = arch.cleanQuery(json)
    this.dbg('find where: %o', Object.keys(query))
   ...
   * @return {_Model}
   */
  static findOrLoad (data) {
    if (typeof data === 'string') { data = { path: data } }
    this.dbg('findOrLoad', data)
    let rec = this.find(data)
    if (!rec) rec = this.readFromSync(data.path)
    return rec
  }
