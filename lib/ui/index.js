...
const ui = module.exports = {
  ...
  async Start (opts = {}) {
    ...
    if (!ui.REPLServer && ui.starting) {
       ...
       **/
      process.emit('pixeldeck:uiStarting')
      ...
      ui.starting = new Promise(function (resolve, reject) {
        ...
        ui.REPLServer.context.models = require('../mdl')
        /**
         * after ui is ready to use
         * @event process:uiStarted
         */
        process.emit('pixeldeck:uiStarted')
        return resolve(ui)
      })
    }
    return ui.starting
  ...
  async Stop () {
     ...
     */
    process.emit('pixeldeck:uiStopping')
     ...
     */
    process.emit('pixeldeck:uiStopped')
